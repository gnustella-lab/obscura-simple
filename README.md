...
Current package revision: `1.177-8` — ready-to-use `.deb` with the simple UI (`obscura-simple_1.177-8_amd64.deb`, ~24M).
...
```bash
./contrib/bin/build-simple-deb.bash   # builds -> ./obscura-simple_1.177-8_amd64.deb
sudo apt install ./obscura-simple_1.177-8_amd64.deb
systemctl status obscura.service      # -> active (running)
sudo obscura add-operator $USER && newgrp obscura
```
